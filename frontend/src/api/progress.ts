export interface SessionPayload {
  ...
  theme:     string
  level:     string
  mode:      'quiz' | 'cards' | 'fill-blank' | 'listen' | 'speak' | 'sentence-builder' | 'dictee' | 'paires' | 'dialogue'
  score:     number   // 0-100
  xp_gained: number
  correct:   number
  total:     number
...
export function calcXp(mode: 'quiz' | 'cards' | 'fill-blank' | 'listen' | 'speak' | 'sentence-builder' | 'dictee' | 'paires' | 'dialogue', correct: number, total: number): number {
